...
import org.beynet.utils.messages.scale.MessageQueueScaleImpl;
...
public class ConstructorImpl implements Constructor {
    ...
    private void configureQueue(MessageQueue result) {
        Field f ;
        try {
            // retrieve databaseaccessor name for this queue
            // ----------------------------------------------
            f= result.getClass().getDeclaredField("accessorName") ;
            f.setAccessible(true);
            String name = (String)f.get(result);
            ...
            // injecting UJB instance of this accessor
            // ---------------------------------------
            f = result.getClass().getDeclaredField("accessor");
            f.setAccessible(true);
            f.set(result, ujbList.get(name));
            ...
            // retrieve RequestManager name for this queue
            // -------------------------------------------
    ...
    public void configure(Object e) {
        ...
        if (e instanceof MessageQueue) {
            configureQueue((MessageQueue)e);
        }
        ...
        for (Field f: fields) {
            ...
            if (ujb!=null) {
                ...
                try {
                    ...
                    logger.error("Error injecting ujb="+ujb.name()+" for target class="+e.getClass().getCanonicalName(),e1);
     ...
     * The DataBaseAccessor and the RequestManager will be injected during the configuration
     * of the queue see {@link ConstructorImpl#configureQueue(MessageQueueImpl)}
     * @param name
     * @param accessorName
     * @param managerName
     * @param scalable
     */
    protected void constructQueue(String name, String accessorName,String managerName,String scalable) {
        MessageQueue result = null ;
        if (scalable==null || "false".equalsIgnoreCase(scalable)) {
            result = (MessageQueueImpl)createUJB(MessageQueueImpl.class, name);
        }
        else {
            result = (MessageQueueScaleImpl)createUJB(MessageQueueScaleImpl.class, name);
        }
        ...
        class QueuesCallBackImpl implements XmlCallBack {
            ...
            public void onNewTagAttributs(List<String> parents, String tagName,
                ...
                if ("queue".equals(tagName)) {
                    ...
                    constructQueue(tagValues.get("name"),tagValues.get("datasource"),tagValues.get("manager"),tagValues.get("scalable"));
                }
            }
        };
